...
if($pages[$link]['add_sort_field'] != '')
	$add_sort_field  = $pages[$link]['add_sort_field'];
else
	$add_sort_field  = $field;

$field = $add_sort_field;

if($pages[$link]['primary_key_column'] != '')
	$primary_key_column	= $pages[$link]['primary_key_column'];
else
	$primary_key_column	= 'id';

$filter = '';
if ( $pages[$link]['sorting_filter'] != '' )
	$filter = ' where ' . $pages[$link]['sorting_filter'];
    ...
    <td colspan="2">
        ...
        <ul id="reOrder">
            ...
            <?php
				...
				$sq = "SELECT * FROM $table ". $filter ." ORDER BY $add_sort_field asc";

                $q = mysqli_query($Conn_db,$sq);

                while( $r = mysqli_fetch_assoc($q) ) {

                    $id = $r[$primary_key_column];
  ...
  $(document).ready(function() {
    ...
    $("#reOrder").sortable({
      ...
      update : function () {
  		...
  		$("#info").load("reOrder.php?tbl=<?=$table?>&fld=<?=$field?>&id=<?php echo $primary_key_column; ?>&" + order);
