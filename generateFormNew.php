...
//		$fileUpd .= "\t" . "\t" .'if ($_FILES["' . $e_name[$i] . '"]["name"] == "") ' . "\n";
	...
	if ($e_extra[$i] == 'Array')
		...
		$phpVars .= 'while ($bc_row = mysqli_fetch_assoc($arrRES) )' . "\n";
...
$dbDnading .= "\t" ."\t" ."\t" .'$res'. "\t" . '='. "\t" .'mysqli_query($arrRES,$sql);' . "\n";
$dbDnading .= "\t" ."\t" ."\t" .'$frmID = mysqli_insert_id($arrRES);' . "\n";
$dbDnading .= "\t" ."\t" ."\t" .'if ($res) {' . "\n";
...
$dbDnading .= '$res'. "\t" . '='. "\t" .'mysqli_query($arrRES,$sql);' . "\n";
$dbDnading .= 'if ($res) {' . "\n";
$dbDnading .= "\t" .'if ($row = mysqli_fetch_assoc($arrRES) ) {' . "\n";
